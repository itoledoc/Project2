...
shaped data for that element. We have also provided a way to save the data in a
file, so that you could use mongoimport later on to import the shaped data into
MongoDB.
...
- if second level tag "k" value does not start with "addr:", but contains ":",
  you can process it same as any other tag.
...
def shape_element(element):
    ...
    if element.tag == "node" or element.tag == "way":
        for atrk, atrv in element.attrib.iteritems():
            if atrk in CREATED:
                node['created'][atrk] = atrv
            elif atrk == 'lon':
                position[1] = float(atrv)
            elif atrk == 'lat':
                position[0] = float(atrv)
            else:
                node[atrk] = atrv
...
def test():
    # NOTE: if you are running this code on your computer, with a larger
    # dataset, call the process_map procedure with pretty=False. The pretty=True
    # option adds additional spaces to the output, making it significantly
    # larger.
    ...
    assert data[-1]["node_refs"] == [
        "2199822281", "2199822390",  "2199822392", "2199822369", "2199822370",
